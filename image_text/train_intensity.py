...
###
from models.classifier import ClassifierLSTM_Intensity
...
def train_loop(trn_idx, val_idx):
    ...
    #                                                 root_dir = CFG.train_path, transform=get_transforms(data = 'train'), task='intensity')
    ...
    #                                                 root_dir = CFG.train_path, transform=None, task='intensity') 
    ...
    test_data = MemoDataset_Emotion(test_images, input_tensor_test_pad, humour_ytest, sarcasm_ytest, offensive_ytest, motivational_ytest, \
                                                    root_dir = CFG.test_path, transform=None, task='intensity') 
    ...
    classifier = ClassifierLSTM_Intensity(CFG.hidden_d, CFG.hidden_d2, CFG.n_intensity_classes[0], \
         CFG.n_intensity_classes[1], CFG.n_intensity_classes[2], CFG.n_intensity_classes[3], CFG.dropout, CFG.n_layers)
    ...
    # Loss and optimizer
    # criterion_humour = nn.CrossEntropyLoss(weight=CFG.class_weight_gradient_intensity_humour)
    # criterion_sarcasm = nn.CrossEntropyLoss(weight=CFG.class_weight_gradient_intensity_sarcasm)
    # criterion_offensive = nn.CrossEntropyLoss(weight=CFG.class_weight_gradient_intensity_offensive)
    # criterion_motivation = nn.CrossEntropyLoss(weight=CFG.class_weight_gradient_intensity_motivation)
